...
- 原始用户嵌入 $U$ 经过一个可插拔的擦除模块 $f(U,\theta)$，得到新嵌入 $\hat{U}$。作者通过编码率控制 $\hat{U}$ 的分布，通过最大化类别编码率 $R^c$ 来破坏用户嵌入中与性别、年龄等属性相关的分布模式，同时使总体编码率 $R$ 维持在合理值 $b$ 附近，从而有效删除敏感属性信息，并保留推荐所需的用户兴趣信息。
