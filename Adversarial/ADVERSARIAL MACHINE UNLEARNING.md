...
#### Stackelberg game 的双层优化
这是一个先手与后手的博弈。

- **上层**：上层 unlearner 最小化 retain loss + 审计项，unlearner 先决定把模型改成什么样
- **下层**：auditor 对当前 𝜃𝑢​选最佳响应，auditor 看到后，用最佳 MIA 来查你
...
- 普通 fine-tune 只关心模型自己的 loss；这里还要关心：我改了模型之后，会不会顺带把攻击者也搞得更难攻击。
### 5.把攻击者最优解写成隐式方程
- auditor 的最优攻击器不是手写公式能直接写出来的。于是就换一种思路：不直接求显式解，而是抓住‘最优解必须满足 KKT 条件’。有了这些条件，就能用隐函数定理做反传，把整个博弈变成可微的。
