---
title: 'LEGO: A Lightweight Multiple-Attribute Unlearning Framework for Recommender Systems.md'

---

# LEGO: A Lightweight Multiple-Attribute Unlearning Framework for Recommender Systems
## 方法
-  本文的任务是解决推荐系统中的多属性动态遗忘问题，它先通过 Embedding Calibration 对每个敏感属性分别进行互信息最小化，利用 vCLUB 估计用户 embedding 与该属性之间的信息泄露，并通过参数空间约束避免 embedding 偏离原始表示太远；然后通过 Flexible Combination 学习多个校准 embedding 的线性组合权重，使最终 embedding 同时降低与多个敏感属性的互信息。
## 公式
...
$$\log \frac{p(x,y)}{p(x)p(y)} = \log \frac{p(y|x)p(x)}{p(x)p(y)} = \log \frac{p(y|x)}{p(y)} = \log p(y|x) - \log p(y)$$
...
$$
\begin{aligned}
I_{\text{CLUB}}(\boldsymbol{x}; \boldsymbol{y}) &= \mathbb{E}_{p(\boldsymbol{x},\boldsymbol{y})} [\log p(\boldsymbol{y} | \boldsymbol{x})] \\
&\quad - \mathbb{E}_{p(\boldsymbol{x})}\mathbb{E}_{p(\boldsymbol{y})} [\log p(\boldsymbol{y} | \boldsymbol{x})].
\end{aligned}
$$
...
* **公式 (5)：** 最小化 $KL(p(\boldsymbol{x}, \boldsymbol{y}) \| q_\phi(\boldsymbol{x}, \boldsymbol{y}))$ 最后等价于最大化 $\mathbb{E}_{p(\boldsymbol{x},\boldsymbol{y})}[\log q_\phi(\boldsymbol{y}|\boldsymbol{x})]$，也就是让预测器 **$q_\phi(\boldsymbol{y}|\boldsymbol{x})$** 在真实配对 **$(\boldsymbol{x}, \boldsymbol{y})$** 上预测得越准越好。
    ...
    &= \min_{\phi} \mathbb{E}_{p(\boldsymbol{x}, \boldsymbol{y})} [\log p(\boldsymbol{y}|\boldsymbol{x})] - \mathbb{E}_{p(\boldsymbol{x}, \boldsymbol{y})} [\log q_\phi(\boldsymbol{y}|\boldsymbol{x})].
    \end{aligned}
    $$
...
* $\text{proj}(\boldsymbol{U}_t)$：投影操作，意思是把超出范围的 $\boldsymbol{U}_t$ 拉回合法范围内。从 U0出发，沿着原来 Ut的方向走，但只走 ϵ这么远。
...
* $\boldsymbol{U}(\boldsymbol{\alpha})$：最终组合出来的新 embedding，可以理解为加权求和：$\boldsymbol{U}(\boldsymbol{\alpha}) = \sum_{i=1}^k \alpha_i \boldsymbol{U}_i^*$。
